    ...
    <div class="container-fluid p-0">
        ...
        if ($_POST) {
            try {
                ...
                if ($updateCustomerStmt->execute()) {
                    echo "<script>window.location.href='customer_detail.php?cus_email='+ '$cus_email' + '&action=profileUpdated';</script>";
                } else {
                    echo "<script>window.location.href='customer_detail.php?cus_email='+ '$cus_email' + '&action=profileUpdateFail';</script>";
                }
            } catch (PDOException $exception) {
                die('ERROR: ' . $exception->getMessage());
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?cus_email={$cus_email}"); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="d-flex justify-content-center">
                ...
                <?php echo "<a href='customer_detail.php?cus_email={$cus_email}' class='actionBtn btn mb-3 mx-2'>Back</a>"; ?>
            </div>
        </form>
    </div>
